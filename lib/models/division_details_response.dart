class DivisionDetailsResponse {
  ...
  DivisionDetailsResponse({required this.status, required this.message, required this.data});
...
class DivisionDetailsData {
  ...
  DivisionDetailsData({required this.division, required this.meta, required this.subscriptionPlan, required this.scales,required this.brands});
  ...
  factory DivisionDetailsData.fromJson(Map<String, dynamic> json) {
    return DivisionDetailsData(
      ...
      meta: (json['meta'] as List?)?.map((item) => Meta.fromJson(item ?? {})).toList() ?? [],
      subscriptionPlan: SubscriptionPlan.fromJson(json['subscription_plan'] ?? {}),
      scales: (json['scales'] as List?)?.map((item) => Scales.fromJson(item ?? {})).toList() ?? [],
      brands: (json['brands'] as List?)?.map((item) => Brands.fromJson(item ?? {})).toList() ?? [],
    );
  }
}
...
class Scales {
  ...
  factory Scales.fromJson(Map<String, dynamic> json) {
    return Scales(
      ...
      s3Url: json['s3_url'] ?? '',
      b64data: json['scale_json']?['b64data'], // Accessing the b64data from the nested scale_json
    );
  }
}
